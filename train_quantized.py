...
import os
import sys
sys.path.insert(1, r"../ThriftyNets")
...
def quantifier_no_prop(w, n_bit,maxi):
...
class QuantizedThriftyNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = quantifier(x, self.n_bits_activ, self.max)
        # print("Quantized x")
        # self.Lconv.weight.data[2][0] = torch.ones(3,3)
        # self.Lconv.weight.data[2][0][1][0] = 1
        # breakpoint()
        # x = quantifier(x, self.n_bits_activ)
        hist = [None for _ in range(self.n_history-1)] + [x]
        # breakpoint()
        for t in range(self.n_iter):
            hist[-1] = quantifier(hist[-1], self.n_bits_activ, self.max)
            a = self.Lconv(hist[-1])
            y = quantifier_no_prop(a, self.n_bits_activ, self.max)
            # print("Quantized y")
            # if self.n_tests == 4:
            breakpoint()
            fm_height = hist[-1].size(-1)
            if self.n_tests > 0:
                with open('conv2d_fm_height.csv', 'a') as f1:
                    f1.write('%d,\n' % fm_height)
                with open('conv2d_in.csv','ab') as f2:
                    for c in range(self.n_filters):
                        np.savetxt(f2, hist[-1].cpu().numpy()[0][c], delimiter=",", fmt="%f", newline=",\n")
                with open('conv2d_out.csv','ab') as f3:
                    for c in range(self.n_filters):
                        np.savetxt(f3, y.cpu().numpy()[0][c], delimiter=",", fmt="%f", newline=",\n")
                self.n_tests -= 1
            else:
                print("Test vectors written")
            ...
            # breakpoint()
            # a = quantifier(a, self.n_bits_activ, self.max)
            # breakpoint()
    ...
    def quantize(self):
        ...
        self.Lconv.weight.data.copy_(quantifier(self.QLconv, self.n_bits_weight, self.max_weight))
